...
class TestPermissions:
    ...
    async def test_superuser_required_access_with_admin(
        ...
        """Test admin access to endpoints requiring superuser permissions"""
        # Test the user list endpoint (typically requires admin permissions)
    ...
    async def test_superuser_required_access_with_normal_user(
        ...
        """Test normal user access to endpoints requiring superuser permissions"""
    ...
    async def test_authenticated_access_with_valid_token(
        ...
        """Test authenticated user access to endpoints requiring authentication"""
        response = await async_client.get(
            "/api/v1/base/userinfo",
            headers={"Authorization": f"Bearer {normal_user_token}"},
        )

        # Authenticated user should be able to access their own info
        assert response.status_code == 200
        data = response.json()["data"]
        assert "username" in data

    async def test_authenticated_access_without_token(self, async_client: AsyncClient):
        """Test unauthenticated user access to endpoints requiring authentication"""
    ...
    async def test_version_endpoint_access(self, async_client: AsyncClient):
        """Test version info endpoint access"""
    ...
    async def test_user_crud_permissions(
        ...
        # Test creating a user (typically requires admin permissions)
        user_data = {
        ...
        # Normal user attempts to create a user
        normal_user_data = {
    ...
    async def test_token_validation_edge_cases(self, async_client: AsyncClient):
        ...
        test_cases = [
            "",  # Empty token
            "Bearer",  # Bearer only
            "Bearer ",  # Bearer followed by space only
