...
class TestJWTAuthentication:
    ...
    def test_token_expiration_times(self):
        ...
        # Check that expiration times match the configuration
        now = datetime.now(UTC)

        # Access token should expire within the configured number of minutes
        access_expected_exp = now + timedelta(
            minutes=settings.JWT_ACCESS_TOKEN_EXPIRE_MINUTES
        )
        access_diff = abs((access_payload.exp - access_expected_exp).total_seconds())
        assert access_diff < 10  # Allow 10-second margin

        # Refresh token should expire within the configured number of days
        refresh_expected_exp = now + timedelta(
            days=settings.JWT_REFRESH_TOKEN_EXPIRE_DAYS
        )
        refresh_diff = abs((refresh_payload.exp - refresh_expected_exp).total_seconds())
        assert refresh_diff < 10  # Allow 10-second margin
