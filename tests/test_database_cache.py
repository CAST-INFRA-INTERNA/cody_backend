...
class TestDatabaseIntegration:
    ...
    async def test_user_model_crud(self):
        ...
        try:
            ...
            raise AssertionError("Should have raised a DoesNotExist exception")
        except Exception:
            # User has been deleted, exception is expected
            pass
    ...
    async def test_database_transaction_rollback(self):
        ...
        # Verify the user count did not increase after rollback
...
class TestCacheIntegration:
    ...
    async def test_cache_manager_connection(self):
        ...
        # Test connection (should degrade gracefully if Redis is unavailable)
    ...
    async def test_cache_decorator_functionality(self):
        ...
        # If cache is available, the second call should use the cache
        result2 = await test_function("test")
        assert result2 == "result_test"

        if cache_manager.redis:
            # When Redis is available, cache should be used
            assert call_count == 1
        else:
            # When Redis is unavailable, the function is called directly
            assert call_count == 2
    ...
    async def test_cache_with_api_endpoints(
        ...
        """Test cache behavior with API endpoints"""
        headers = {"Authorization": f"Bearer {admin_token}"}

        # Call the user info endpoint multiple times
        responses = []
        for _ in range(3):
