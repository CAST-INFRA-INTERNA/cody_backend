...
class DataProcessor:
    """Data processing utility class - merge all duplicate data processing logic"""

    @staticmethod
    def extract_workflow_data(chunks: list[str]) -> dict | None:
        """Extract workflow_finished event data from data chunks - unified version"""
        logger.info(f"Start extracting workflow_finished event from {len(chunks)} data chunks")

        found_event_types = []

        # Search from back to front, the latest event is at the back
                ...
                if json_content_str and json_content_str != "[DONE]":
                    try:
                        ...
                        if event_type == "workflow_finished":
                            logger.info(
                                f"Found workflow_finished event in data chunk {len(chunks) - 1 - i}"
                            )
                            return event_data
            ...
            if chunk.startswith("data:"):
                ...
                if json_content_str and json_content_str != "[DONE]":
                    try:
                        ...
                        # Extract text from different types of events
