...
class SensitiveFilterHandler:
    """Unified sensitive word processor"""
    ...
    def check_input(self, text: str) -> tuple[bool, str | None]:
        ...
        Returns:
            Tuple[bool, Optional[str]]: (Whether it contains sensitive words, matched sensitive words)
        """
    ...
    async def handle_sensitive_input_stream(self, matched_word: str, query: str):
        ...
        logger.warning(f"User input contains sensitive words '{matched_word}': {query[:100]}")

        async def sensitive_word_response():
            # Send sensitive word warning
            error_event = {
                "event": "error",
    ...
    def filter_chunk(self, chunk: str) -> str | None:
        """Filter sensitive words in data chunks"""
