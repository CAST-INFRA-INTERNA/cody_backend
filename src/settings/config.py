...
class Settings(BaseSettings):
    ...
    VERSION: str = "0.1.0"
    APP_TITLE: str = os.getenv("APP_TITLE", "Cody")
    PROJECT_NAME: str = os.getenv("PROJECT_NAME", "Cody")
    ...
    DB_ENGINE: str = "postgres"  # Default: PostgreSQL
    ...
    def parse_company_role_mapping(cls, v):
        """Parse the COMPANY_ROLE_MAPPING environment variable"""
        if isinstance(v, str):
            try:
                return json.loads(v)
    ...
    def validate_db_password(cls, v):
        ...
        # Allow empty password in test and development environments
    ...
    def validate_swagger_password(cls, v):
        ...
        # Allow empty password in test environment
    ...
    def _validate_production_config(self):
        ...
        if self.DB_ENGINE == "sqlite":
            raise ValueError("PostgreSQL is recommended over SQLite in production environment")

        if "localhost" in self.CORS_ORIGINS:
            raise ValueError("CORS access from localhost should not be allowed in production environment")
