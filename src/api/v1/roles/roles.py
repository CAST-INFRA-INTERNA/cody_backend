...
@router.get("/list", summary="View Role List", response_model=RoleListResponse)
async def list_role(
    page: int = Query(1, description="Page Number"),
    page_size: int = Query(10, description="Page Size"),
    role_name: str = Query("", description="Role Name for query"),
):
    q = Q()
    if role_name:
...
@router.get("/get", summary="View Role", response_model=RoleDetailResponse)
async def get_role(
...
@router.get("/authorized", summary="View Role Permissions", response_model=RoleAuthorizedResponse)
