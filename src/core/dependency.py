...
class AuthControl:
    ...
    async def is_authed(
        ...
        try:
            # Directly use the token provided by HTTPBearer (the Bearer prefix has been removed)
            ...
            raise HTTPException(status_code=401, detail="Login expired") from e
        except Exception as e:
            # Record detailed error information to logs, but do not return to the user
...
class PermissionControl:
    ...
    async def has_permission(
        ...
        """Check if the user has permission to access the specified API
        ...
        Raises:
            HTTPException: Throw 403 error when user has no permission
        """
        ...
        # Check permission matching (supports path parameters)
...
class AgentPermissionControl:
    ...
    async def has_agent_permission(
        ...
        """Check if the user has permission to access the agent
        ...
        raise HTTPException(status_code=403, detail="No permission to access the agent")
    ...
    async def filter_agents_by_permission(
        ...
        """Get the set of agent IDs that the user has permission to access

        Args:
            current_user: Current authenticated user

        Returns:
            set[int]: Set of agent IDs the user has permission to access, super admin returns empty set indicating no limit
        """
        # Super administrator has all permissions, returning empty set indicating no limit
        if current_user.is_superuser:
            return set()
