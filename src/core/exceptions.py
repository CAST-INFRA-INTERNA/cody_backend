...
async def DoesNotExistHandle(req: Request, exc: DoesNotExist) -> JSONResponse:
    # Record detailed error information to logs
    error_details = {
        "method": req.method,
        "url": str(req.url),
        "path": req.url.path,
...
async def IntegrityHandle(request: Request, exc: IntegrityError):
    # Record data integrity error details
    error_details = {
        "method": request.method,
        "url": str(request.url),
        "path": request.url.path,
...
async def RequestValidationHandle(
    ...
    # Record request validation error details
    error_details = {
        "method": request.method,
        "url": str(request.url),
        "path": request.url.path,
...
async def ResponseValidationHandle(
    ...
    # Record response validation error details
    error_details = {
        "method": request.method,
        "url": str(request.url),
        "path": request.url.path,
...
async def UnhandledExceptionHandle(request: Request, exc: Exception) -> JSONResponse:
    ...
    # Record detailed information of unhandled exceptions
    error_details = {
        "method": request.method,
        "url": str(request.url),
        "path": request.url.path,
