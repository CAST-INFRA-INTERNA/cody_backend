...
def register_exceptions(app: FastAPI):
    ...
    # Register generic exception handler (must be placed last as a fallback)
    app.add_exception_handler(Exception, UnhandledExceptionHandle)
    # Register rate limit exception handling
...
async def init_menus():
    logger.info("🔧 Initializing system menus...")
...
async def init_db():
    ...
    try:
        ...
        # Do not automatically delete migrations folder to avoid accidental loss of migration history
...
async def init_roles():
    ...
    if not roles:
        ...
        user_role = await Role.create(
            name="Ordinary User",
            desc="Ordinary User Role",
        )

        # Assign all APIs to administrator role
        all_apis = await Api.all()
        await admin_role.apis.add(*all_apis)
        # Assign all menus to administrator and ordinary user
        all_menus = await Menu.all()
        await admin_role.menus.add(*all_menus)
        await user_role.menus.add(*all_menus)

        # Assign basic APIs to ordinary user
        basic_apis = await Api.filter(Q(method__in=["GET"]) | Q(tags="Basic Module"))
        await user_role.apis.add(*basic_apis)

        logger.info("✅ User roles initialized successfully - Roles: Administrator, Ordinary User")
    else:
...
async def init_data():
    ...
    logger.info("🔄 Initializing basic data in parallel...")
...
async def startup():
    """App startup event"""
    logger.info("🚀 Fast API app starting...")
...
async def shutdown():
    """App shutdown event"""
    logger.info("🛑 Fast API app shutting down...")
...
async def init_app(app: FastAPI):
    ...
    logger.info("🎉 Fast API app startup completed!")
