...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        # Set looser CSP policy for Swagger UI and ReDoc
        ...
        # Add HSTS header only in HTTPS environment
...
class HttpAuditLogMiddleware(BaseHTTPMiddleware):
    ...
    async def get_request_args(self, request: Request) -> dict:
        ...
        if request.method in ["POST", "PUT", "PATCH"]:
            ...
            # If file upload request, skip request body parsing
    ...
    async def get_response_body(self, request: Request, response: Response) -> Any:
        # Do not record response body for streaming responses
        if isinstance(response, StreamingResponse):
            return {"message": "[Streaming Response]"}

        # Check response type, skip if streaming related
        ...
        if any(request.url.path.startswith(path) for path in self.audit_log_paths):
            try:
                ...
                # Keep only basic info, remove detailed response content
...
class RequestLoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        try:
            ...
            # Record detailed request exception information
