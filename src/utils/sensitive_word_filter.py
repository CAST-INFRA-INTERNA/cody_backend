...
Uses the AhoCorasick algorithm for efficient sensitive word detection and filtering.
"""
...
class SensitiveWordFilter:
    ...
    Implements efficient sensitive word detection using the AhoCorasick algorithm.
    """
    ...
    def _build_automaton(self) -> None:
        """Build the AhoCorasick automaton

        Builds an automaton from the configured sensitive word list for fast matching.
        """
        if not self.enabled:
            logger.info("Sensitive word filter is disabled")
            return
        ...
        try:
            ...
            logger.info(
                f"Sensitive word filter initialized, loaded {len(settings.SENSITIVE_WORDS)} words"
            )
    ...
    def contains_sensitive_word(self, text: str) -> tuple[bool, str | None]:
        """Detect whether text contains sensitive words

        Args:
            text: The text to check

        Returns:
            Tuple[bool, Optional[str]]: (whether sensitive word found, matched word)
        """
    ...
    def filter_text(self, text: str) -> str:
        ...
        Returns:
            str: Filtered text; if sensitive words are found, returns the configured response message
        """
        if not text:
            return text
        ...
        if contains_sensitive:
            logger.info(f"Text contains sensitive word '{matched_word}', returning response message")
            return self.response_message

        return text

    def filter_streaming_chunk(self, chunk: str) -> str | None:
        """Filter a streaming output chunk

        Args:
            chunk: A streaming output data chunk

        Returns:
            Optional[str]: Filtered chunk; returns None to block output if sensitive words are found
        """
        ...
        try:
            ...
            if chunk.startswith("data:"):
                ...
                if json_content_str and json_content_str != "[DONE]":
                    try:
                        ...
                        # Check the 'answer' field (usually contains AI reply content)
                        ...
                        if text_to_check:
                            ...
                            if contains_sensitive:
                                logger.warning(
                                    f"Sensitive word detected in streaming output: '{matched_word}', blocking output"
                                )
                                # Return None to block output
                                return None

                    except json.JSONDecodeError:
                        # Not JSON format: check the raw text directly
            ...
            logger.error(f"Failed to filter streaming chunk: {str(e)}")
            return chunk
