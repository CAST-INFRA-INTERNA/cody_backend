...
class LoggingConfig:
    ...
    def _json_default(value: Any) -> Any:
        """Default processing logic for JSON serialization"""
    ...
    def _build_log_entry(self, record: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Support context pass-through, compatible with request_id / user_id etc. fields
    ...
    def setup_logger(self):
        ...
        # Enable unified patcher, ensure all log output is JSON structure
        ...
        loguru_logger.add(
            sink=f"{self.log_dir}/backend_{{time:DD-MM-YYYY}}.log",
            level="DEBUG",
            format="{extra[serialized]}",
            rotation="100 MB",
            retention="30 days",
        ...
        loguru_logger.add(
            sink=f"{self.log_dir}/backend_error_{{time:DD-MM-YYYY}}.log",
            level="ERROR",
            format="{extra[serialized]}",
            rotation="50 MB",
            retention="90 days",
        ...
        # Critical error log (CRITICAL level)
        loguru_logger.add(
            sink=f"{self.log_dir}/backend_critical_{{time:DD-MM-YYYY}}.log",
            level="CRITICAL",
            format="{extra[serialized]}",
            rotation="10 MB",
            retention="180 days",
        ...
        # Note: Rebinding here creates new logger instance

        # Record log system startup
        loguru_logger.bind(event="logger_startup").info("Log system started")
