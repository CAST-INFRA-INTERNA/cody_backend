...
Extracts route information from FastAPI application to generate API documentation
"""
...
try:
    ...
    print(f"Cannot import FastAPI application: {e}")
    app = None
...
def get_model_fields(model: type[BaseModel]) -> list[dict[str, Any]]:
    ...
    for field_name, field_info in model.model_fields.items():
        ...
        if hasattr(field_info, "min_length") and field_info.min_length is not None:
            constraints.append(f"Minimum length: {field_info.min_length}")
        if hasattr(field_info, "max_length") and field_info.max_length is not None:
            constraints.append(f"Maximum length: {field_info.max_length}")
        if hasattr(field_info, "ge") and field_info.ge is not None:
            constraints.append(f"Minimum value: {field_info.ge}")
        if hasattr(field_info, "le") and field_info.le is not None:
            constraints.append(f"Maximum value: {field_info.le}")
    ...
    if endpoint:
        ...
        for param_name, param in sig.parameters.items():
            ...
            if param.annotation != param.empty:
                ...
                # Handle generic and Union types
                if hasattr(param.annotation, "__origin__"):
                    if param.annotation.__origin__ is Union:
                        # Handle Union types, such as Optional[str]
...
def generate_parameter_table(parameters: list[dict[str, Any]]) -> str:
    ...
    table = "| Parameter | Type | Location | Required | Description | Default |\n"
    table += "|-----------|------|----------|----------|-------------|---------|\n"
...
def generate_request_body_section(request_body: dict[str, Any]) -> str:
    ...
    if "model_fields" in request_body:
        ...
        content += "| Field | Type | Required | Description | Example | Constraints |\n"
        content += "|-------|------|----------|-------------|---------|-------------|\n"
...
API interface documentation related to {module_display_name}.
...
"""
    ...
    for route_data in routes:
        ...
        if method == "GET":
            if query_params:
                content += "params = {\n"
                for param in query_params[:3]:  # Show only first 3 parameters as example
                    if param.get("example"):
...
def extract_route_info(app: FastAPI) -> dict[str, list[Any]]:
    """Extract route information, return original APIRoute objects"""
    if app is None:
        return {}
    ...
    for route in app.routes:
        if isinstance(route, APIRoute):
            # Extract path module information
...
Most APIs require JWT authentication. First, obtain an access token through the login interface, then include it in the request header:
...
- [File Management](files.md) - File upload/download
...
After starting the service, you can access interactive API documentation at:
...
API interface documentation related to {module.title()}.
...
"""
        ...
        print("FastAPI application unavailable, generating basic documentation structure...")
